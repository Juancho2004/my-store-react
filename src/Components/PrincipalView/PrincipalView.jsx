...
export function PrincipalView() {
  const { activeTab, setActiveTab, filterCategories, dataFilter } =useTabCategories();
  const { products, limit, handleLimits, handleAll } = useFetchProducts();
  const { details, handleProductClick, handleCloseModal, modalVisible } = useFetchProductId();
  const { searchValue, setSearchValue, searchproducts, handleSearch, handleSubmit } = useSearchProducts({products});
  const location = useLocation();
  const { user } = location.state || {};
  const {userDetails} = useUserID(user);
  const { product, productQuantity, totalPrice, car } = useUserCar(user);
  const { handleShowModal, handleCloseShowModal, showModal, handleShowModalCart, handleCloseShowModalCart, showModalCart, length } = useLogic(product);
